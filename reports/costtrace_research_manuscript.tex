...
\begin{itemize}
  ...
  \item Replace all provisional wording if Phase 04--05 upgrades produce new validated multi-fold or sensitivity results.
...
Household overlap & The legacy node-level split placed train and test nodes in the same household for 37/88 households. & Use household-level split with no household overlap. & The small number of households can increase metric variance; multi-fold evaluation remains future work. \\
...
The graph-learning model is a lightweight pure-PyTorch Weighted GraphSAGE classifier. After leakage-control revisions, it uses 9 input features: train-scaled degree centrality, train-scaled weighted degree, train-scaled betweenness centrality, train-scaled closeness centrality, sleep-room exposure, caregiving exposure, scaled age group, sex, and site.

Edge weights are transformed as \(\log(1 + \mathrm{duration})\) and row-normalized for weighted mean aggregation. The model is trained with the household-held-out 70\%/15\%/15\% train/validation/test protocol described above, 500 epochs, Adam optimization, learning rate 0.01, weight decay \(5\times 10^{-4}\), and seed 42. The output probability is used as a ranking score for top-\(k\) selection.
...
Under the leakage-controlled household-held-out protocol, the Weighted GraphSAGE proxy achieves test AUC 0.4428 and test average precision 0.7359. This is substantially weaker than the previous node-level evaluation and indicates that the original course-style protocol overestimated generalization. The model output is retained only as one ranking signal for the intervention comparison, not as evidence of a deployable diagnostic classifier.
...
\midrule
Train & 0.6398 & 0.8198 & 0.8247 & 0.7017 & 1.0000 \\
Validation & 0.5676 & 0.7794 & 0.8315 & 0.7115 & 1.0000 \\
Test & 0.4428 & 0.7359 & 0.8506 & 0.7400 & 1.0000 \\
\bottomrule
\end{tabular}
\end{table}
